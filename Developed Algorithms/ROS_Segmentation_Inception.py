...
class RosTensorFlow():
    ...
    def callback(self, image_msg):
        ...
        objects=Segmentation.charge(numBoxes,cv_image)

        for i in range(numBoxes):
                Imgs[i]=[i, objects[i], self._session, self.score_threshold, self.use_top_k]


        res =tuple(map(Segmentation.classification,Imgs)) 
